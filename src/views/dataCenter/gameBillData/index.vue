...
<script setup name="gameBillData">
...
onMounted(async () => {
    try {
        ...
        state.searchTime = query.endTime ? [query.startTime, query.endTime] : []
        state.formData = {
            merchantId: state.merchantId,
            receiveTime: state.searchTime.length ? state.searchTime : getTimeRange(),
        }
        fetchData()
    } catch (error) {
