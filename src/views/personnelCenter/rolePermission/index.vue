<template>
    ...
    <div class="container role-permission">
        <el-table class="table-box" :data="state.merchantData" border>
        ...
        <Pagination v-show="state.merchantTotal > 0" :total="state.merchantTotal" v-model:page="state.listQuery._page"
            v-model:size="state.listQuery._size" @pagination="merchantFetchData" />
    </div>
</template>
...
const merchantFetchData = () => {
    let params = { ...state.listQuery, type: 1 }
    globalVBus.$emit('globalLoading', true)
    roleListApi(params).then(res => {
        let { total, list } = res.data
        state.merchantTotal = total || 0
        state.merchantData = list || []
