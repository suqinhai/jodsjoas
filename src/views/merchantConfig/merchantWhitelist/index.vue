<template>
    ...
    <div class="container">
        ...
        <el-table :data="state.tableData" border class="table-box">
            ...
            <el-table-column prop="type_" label="来源" min-width="120" align="center" />
            ...
            <el-table-column prop="emp.name" label="添加人" min-width="140" align="center" />
...
import { getMerchantWhitelistList, deleteMerchantWhitelistConfig } from '@/api/merchantConfig.js'
...
const handleDelete = ({ group, id }) => {
    ElMessageBox.confirm('确定要删除此商户白名单配置吗？', '提示', {
        confirmButtonText: '确 定',
        cancelButtonText: '取消',
        type: 'warning',
    }).then(() => {
        globalVBus.$emit('globalLoading', true)
        deleteMerchantWhitelistConfig({ id, merchantId: group.id, type: 0 }).then(res => {
            fetchData()
        }).catch(err => {
            globalVBus.$emit('globalLoading', false)
        })
