    ...
    <div class="container theme-template-manage">
        ...
        <el-form ref="deployForm" :disabled="state.isDisabled" :rules="state.rules" :model="state.formData" label-width="100px">
            <el-form-item label="商户ID:">
                ...
                <div style="color: red">当前配置的商户可设置所有支付通道，未配置的商户只能设置特定支付通道！</div>
            </el-form-item>
        </el-form>
        <div class="foot">
...
import { getMerchantPayChannelLimit, postMerchantPayChannelLimit } from '@/api/merchantConfig'
...
const getData = () => {
    ...
    getMerchantPayChannelLimit().then(res => {
...
const handleSave = () => {
    globalVBus.$emit('globalLoading', true)
    postMerchantPayChannelLimit(state.formData).then(() => {
        getData()
    }).catch(() => {
        globalVBus.$emit('globalLoading', false)
    })
