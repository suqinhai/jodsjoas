...
const handleBeforeUpload = (rawFile) => {
    ...
    if (props.accept || systemLimit.value.fileTypes) {
        ...
        if (systemLimit.value.fileTypes && !systemLimit.value.fileTypes.some(item => fileType === item)) {
            ElMessage.error(`请上传${typeData.join('/')}格式文件`)
            file.status = 'fail'
            return false
        }
    ...
    if (systemLimit.value.singleFileMax && systemLimit.value.singleFileMax < rawFile.size) {
        ElMessage.error(`文件大小不能超过${parseInt(systemLimit.value.singleFileMax / 1024)}kb`)
        file.status = 'fail'
        return false
    }
    if (rawFile.size / 1024 > props.size) {
        ElMessage.error(`文件大小不能超过${props.size}kb`)
        file.status = 'fail'
        return false
    }
...
const handleDelete = (file) => {
    if (file.status === 'ready') {
        ElMessage.warning('上传中的文件无法执行删除操作')
        return false
    }
