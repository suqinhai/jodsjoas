...
watch(
    ...
    (val) => {
        ...
        if (val && !fileList.value.length && isInit.value) {
            isInit.value = false
            // 首先将值转为数组
...
const handleBeforeUpload = (rawFile) => {
    ...
    if (props.accept || systemLimit.value.fileTypes) {
        ...
        if (!systemLimit.value.fileTypes.some((item) => fileType === item)) {
            ElMessage.error(`请上传${systemLimit.value.value.fileTypes.join('/')}格式图片`)
            file.status = 'fail'
            return false
        }
        if (!typeData.some((item) => fileType === item)) {
            ElMessage.error(`请上传${typeData.join('/')}格式图片`)
            file.status = 'fail'
            return false
        }
    }
    if (systemLimit.value.singleFileMax && systemLimit.value.singleFileMax < rawFile.size) {
        ElMessage.error(`图片大小不能超过${parseInt(systemLimit.value.singleFileMax / 1024)}kb`)
        file.status = 'fail'
        return false
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            img.onload = () => {
                ...
                if (valid) {
                    ...
                    let msg = `请上传尺寸为${props.width}*${props.height}或宽高比为${arrowMaxRation} - ${arrowMinRation}的图片`
                    ElMessage.error(msg)
                    file.status = 'fail'
                    reject()
                }
...
const handleDelete = (file) => {
    if (file.status === 'ready') {
        ElMessage.warning('上传中的图片无法执行删除操作')
        return false
    }
...
const picturepPreview = (index = 0) => {
    if (!fileList.value[index] || !fileList.value[index].url) {
        return ElMessage.warning('文件不存在！')
    }
