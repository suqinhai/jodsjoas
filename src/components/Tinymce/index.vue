...
const state = reactive({
    init: {
        ...
        toolbar: 'undo redo ' +
            ...
            'fullscreen preview ' +
            'image media link codesample code',  // 工具栏配置，设为false则隐藏
        font_size_formats: '10px 12px 14px 16px 18px 20px 22px 24px 28px 32px 36px 48px 56px 72px',  // 字体大小
        // font_family_formats: '微软雅黑=Microsoft YaHei,Helvetica Neue,PingFang SC,sans-serif;苹果苹方=PingFang SC,Microsoft YaHei,sans-serif;宋体=simsun,serif;仿宋体=FangSong,serif;黑体=SimHei,sans-serif;Arial=arial,helvetica,sans-serif;Arial Black=arial black,avant garde;Book Antiqua=book antiqua,palatino;',  //字体样式
        ...
        width: props.width,  // 注：引入autoresize插件时，此属性失效
        ...
        images_upload_handler: (blobInfo) => new Promise((resolve, reject) =>
            fileUpload('image', blobInfo.blob(), resolve, reject)
        ),
            ...
            input.onchange = function () {
                ...
                if (meta.filetype == 'media') {
                    if (file.type.indexOf('mp4') >= 0) {
                        ...
                        return ElMessage.error('媒体上传只支持MP3/MP4格式的文件！')
                    }
                }
...
const verifyFile = (file, singleFileMax, fileTypes, isImage) => {
    ...
    if (singleFileMax && singleFileMax < file.size) {
        valid = false
        ElMessage.error(`文件大小不能超过${parseInt(singleFileMax / 1024 / 1024)}M`)
    }
    return valid
}
